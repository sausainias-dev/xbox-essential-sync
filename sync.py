...
def extract_available_games(page, expected: int) -> dict[str, dict]:
    """Parse the rendered DOM with BeautifulSoup and keep only Available Games."""
    html = page.content()
    ...
    for h in soup.find_all(["h2", "h3"]):
        ...
        if re.search(r"^Available Games\s*\(\d+\)", text, re.I):
            heading = h
            break
    if heading is None:
    ...
    games = {}
    # find_all_next() follows actual rendered document order, regardless of
    # table/card nesting. Stop when the next section heading begins.
    for node in heading.find_all_next():
        if isinstance(node, Tag) and node.name in {"h2", "h3"} and node is not heading:
            break
        ...
        name, year = clean_title(node.get_text(" ", strip=True))
        if not name:
            continue
    ...
    actual = len(games)
    if actual != expected:
        print(f"  DEBUG: rendered Available Games anchors = {actual}")
        raise RuntimeError(
...
def extract_section_links(page, heading_name: str) -> set[str]:
    ...
    heading = None
    for h in soup.find_all(["h2", "h3"]):
        if clean(h.get_text(" ", strip=True)).lower() == heading_name.lower():
            heading = h
            break
    if heading is None:
        return set()
